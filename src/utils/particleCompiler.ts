...
export function getOptimalParticleCount(baseCount: number = 4000): number {
    ...
    if (isMobile && isLowEnd) {
        return Math.floor(baseCount * 0.25); // 1000 for base 4000
    }
    if (isMobile) {
        return Math.floor(baseCount * 0.5); // 2000 for base 4000
    }
    if (isLowEnd || isHighDPI) {
        return Math.floor(baseCount * 0.625); // 2500 for base 4000
    }
    return baseCount;
}
...
export const DEFAULT_PARTICLE_FORMULA: ParticleFormula = {
    ...
    particleCount: 4000,  // 注意: 实际使用时通过 getOptimalParticleCount() 调整
    colorHex: '#ffffff',
    timeScale: 0.0002
};
