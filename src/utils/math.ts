...
import { safeCompileExpression, validateExpression } from './safeEval';
import { PATH_LENGTH_EPSILON } from '../constants/config';
...
export const resamplePath = (points: Point[], targetCount: number): Complex[] => {
    ...
    if (totalLen < PATH_LENGTH_EPSILON) {
        // Safe fallback for degenerate path
        const fallbackPoint = points[0] || { x: 0, y: 0 };
        return Array(targetCount).fill({ re: fallbackPoint.x, im: fallbackPoint.y });
    }
    ...
    for (let i = 0; i < targetCount; i++) {
        ...
        let t = 0;
        if (segmentLen > Number.EPSILON) {
            t = Math.min(1, Math.max(0, (targetDist - distStart) / segmentLen));
        }
 ...
 * Uses FFT optimization for large inputs (O(N log N) instead of O(N²))
 */
export { dft } from './fft';
...
export const generateFromFunction = (
    ...
    // Create functions from strings safely using validated compilation
    // First validate expressions before compilation
    const validation1 = validateExpression(fn1Str, ['t']);
    const validation2 = validateExpression(fn2Str, ['t']);

    if (!validation1.valid || !validation2.valid) {
        console.error("Invalid function strings:", validation1.error || validation2.error);
        return [];
    }

    const fn1 = safeCompileExpression<(t: number) => number>(
        fn1Str,
        ['t'],
        () => 0
    );
    const fn2 = safeCompileExpression<(t: number) => number>(
        fn2Str,
        ['t'],
        () => 0
    );
