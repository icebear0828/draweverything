...
| Primitive | 触发词 | 用途 | 路径 |
|-----------|--------|------|------|
| `health-check` | `/health` | 系统健康检查 | `_primitives/health-check/` |
...
### 🆕 Conditional Composer 模式 (v3.5+)

> [!TIP]
> 对于高成本操作 (如视频生成)，使用 dry-run 模式分离验证与执行。
...
flags:
  - name: --video
    description: "是否生成视频 (默认仅生成 prompt)"
    default: false
  - name: --dry
    description: "仅输出 prompt，不执行生成"
    default: false
---
```
...
| `image-generation/watercolor.md` | 水彩风格 | 艺术插画 |
| `image-generation/cyberpunk.md` | 赛博朋克风格 | 科幻场景 |
| `video-generation/cinematic-motion.md` | 电影运镜 | Veo 视频 |
| `video-generation/smooth-transition.md` | 平滑过渡 | 延时摄影 |
| `video-generation/veo-prompt-guide.md` | Veo 提示词指南 | 视频生成最佳实践 |
...
### Phase 2: Design
Plan the file structure: `.agent/skills/{category}/<skill-name>/`
...
**自动触发**：
```
/rlaif .agent/skills/{category}/<新生成的skill目录>
```
...
### Phase 6: 索引注册 (Auto-Registration)

> [!IMPORTANT]
> 新 Skill 创建后**必须**自动注册到 SKILLS_INDEX.md，确保可被发现。

**注册步骤**：
1. 打开 `.agent/skills/SKILLS_INDEX.md`
2. 根据 Skill 类别定位对应区域：
   - `_primitives/` → Primitives 区域
   - `_meta/` → Meta Skills 区域
   - `multimedia/` → Multimedia 区域
   - `content/` → Content 区域
   - `visual/` → Visual 区域
   - `ai-tools/` → AI Tools 区域
3. 在对应区域表格末尾追加新条目：
   ```markdown
   | [{skill-name}](./{category}/{skill-name}/SKILL.md) | `/{trigger}` | {description} | {dependencies} |
   ```
4. 保存文件

**注册模板**：
```markdown
| [new-skill](./category/new-skill/SKILL.md) | `/trigger` | 描述 | 依赖 |
```

> [!NOTE]
> 如果 Skill 类别不明确，默认放入 `📦 待整理` 区域。
