---
trigger: "/rlaif|/score|优化skill|改进技能|skill quality|技能质量评估|评估skill|技能打分"
description: "使用 RLAIF 模式评估和优化 Skills - 包含八维评分模型和自动修复功能"
category: _meta
---

# RLAIF Optimizer (Ralph Loop) v2.2
...
## Phase 4.5: 变更预览 (Diff Preview)

> [!CAUTION]
> 在应用任何修改前，**必须**展示完整差异并等待用户确认，防止越改越差！

### 4.5.1 Dry-Run 模式

使用 `--dry-run` 参数可以只生成预览而不实际修改：
...
## Phase 7: 一键回滚 (Rollback)

> [!TIP]
> 当发现优化后质量下降时，可以快速回滚到历史版本。

### 7.1 回滚命令

```
/rlaif rollback <目标技能> --to=vX.Y.Z

示例：
/rlaif rollback comic-strip-generator --to=v2.0.0
/rlaif rollback xiaohongshu-copywriter --to=v1.3.0
```

### 7.2 执行步骤

1. 验证目标版本存在：`versions/vX.Y.Z/SKILL.md`
2. 备份当前版本到 `versions/v{当前}/`
3. 复制目标版本到根目录
4. 更新 CHANGELOG.md 记录回滚操作
5. 更新 EVALUATION_LOG.md 记录回滚原因
...
📦 当前版本: v2.2.0 → 备份到 versions/v2.2.0/
📥 目标版本: v2.0.0 → 恢复到根目录
📝 CHANGELOG.md 已更新
📋 EVALUATION_LOG.md 已追加回滚记录

✅ 回滚完成: comic-strip-generator v2.2.0 → v2.0.0
```

### 7.4 回滚日志格式

在 EVALUATION_LOG.md 中追加：
